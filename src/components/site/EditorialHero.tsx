...
export function EditorialHero({
  ...
  return (
    <section className="relative w-full px-3 sm:px-6 lg:px-10 pt-24 pb-8 lg:pt-20 lg:pb-12">
      {/* HERO — full-bleed portrait, headline and CTAs */}
      <div className="mx-auto flex w-full max-w-[92rem] flex-col justify-center">
        ...
        <div className="relative z-30 mt-5 flex flex-col">
          ...
          <div className="ed-rise mt-6 flex shrink-0 flex-wrap items-center justify-center gap-3" style={{ animationDelay: "520ms" }}>
            <button
              type="button"
              onClick={onExplore}
      ...
      {/* BELIEF & VISION — two premium glass cards directly under the hero */}
      <div className="mx-auto mt-8 grid w-full max-w-[92rem] gap-4 sm:grid-cols-2 lg:mt-12 lg:gap-6">
        {columns.map((c, i) => (
          <article
            key={c.t}
            className="ed-rise ed-card p-6 transition-all duration-500 hover:-translate-y-1 lg:p-8"
            style={{ animationDelay: `${220 + i * 90}ms` }}
          >
            <p className="ed-eyebrow">{c.k}</p>
            <h2 className="mt-2 font-display text-xl lg:text-2xl font-bold">{c.t}</h2>
            <p className="mt-3 text-[14px] leading-relaxed text-muted-foreground whitespace-pre-line">{c.v}</p>
          </article>
        ))}
      </div>

      {(hasMore || socials.instagram || socials.youtube) && (
        <div className="ed-rise mx-auto mt-6 flex w-full max-w-[92rem] items-center justify-center gap-2" style={{ animationDelay: "460ms" }}>
          {hasMore && (
            <button
...
function HeroFrame({ image, clips, alt, onReady }: { image: string; clips: string[]; alt: string; onReady?: () => void }) {
  ...
  return (
    <div
      ref={frameRef}
      className="ed-rise ed-frame light-sweep relative mx-auto aspect-[3/4] w-full max-w-[36rem] sm:max-w-[44rem] lg:aspect-auto lg:h-[58svh] lg:max-w-[52rem]"
