...
import { DEFAULT_WHATSAPP_TEMPLATE, buildWaUrl } from "@/lib/whatsapp-template";
...
function ApprovalsTab({ rows, onApprove, reload }: { rows: any[]; onApprove: any; reload: () => void }) {
  ...
  const act = async (id: string, approve: boolean) => {
    setBusy(id);
    try {
      const result: any = await onApprove({ data: { enrollmentId: id, approve } });
      if (approve) {
        // Approval confirmed — automatically send the configured WhatsApp
        // confirmation message (with the registration/ticket details) to the
        // registered student's mobile number. No message is sent on reject.
        const enr = result?.enrollment;
        const ticket = result?.ticketCode ?? enr?.ticket_code ?? null;
        const waUrl = buildWaUrl(enr, ticket, waTemplate, whatsapp);
        if (waUrl && typeof window !== "undefined") {
          window.open(waUrl, "_blank", "noopener,noreferrer");
        } else if (typeof window !== "undefined") {
          toast.error("Approved, but couldn't build the WhatsApp link — check the student's phone number.");
        }
      }
      await reload();
    } catch (e: any) {
  ...
  return (
    <div className="mt-8 space-y-3">
      <p className="text-sm text-muted-foreground">
        Review each uploaded payment screenshot, then approve to issue the ticket, increment seats, and automatically send the WhatsApp confirmation to the student.
      </p>
      {pending.length === 0 && (
