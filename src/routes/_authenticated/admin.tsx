...
function WorkshopsTab({ rows, onSave, onDel, onPub, reload }: any) {
  ...
  const secSessions = (
    <div className="space-y-3">
      ...
      {(f.session_schedule ?? []).map((s: any, i: number) => (
        <div key={i} className="rounded-xl border border-border bg-muted/30 p-3 space-y-2">
          <div className="flex items-center justify-between">
            <p className="text-[11px] uppercase tracking-widest text-muted-foreground">Session {i + 1}</p>
            <button type="button"
              onClick={() => setF({ ...f, session_schedule: (f.session_schedule ?? []).filter((_: any, j: number) => j !== i) })}
              className="px-2 py-1 rounded-lg border border-border text-destructive text-xs hover:bg-destructive/10 active:scale-95 transition">Remove</button>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-[minmax(0,0.8fr)_minmax(0,1.6fr)] gap-2">
            <FieldRow label="Time">
            ...
            <FieldRow label="Song / Class name">
      ...
      {(f.session_schedule ?? []).length === 0 && <p className="text-xs text-muted-foreground">No sessions added yet.</p>}
      <button type="button"
        onClick={() => setF({ ...f, session_schedule: [...(f.session_schedule ?? []), { time: "", name: "" }] })}
        className="w-full sm:w-auto px-4 py-2 rounded-lg border border-primary/40 bg-primary/5 text-primary text-sm font-medium hover:bg-primary/10 transition">＋ Add Session</button>
    </div>
  );
    ...
    <div className="space-y-2">
      {!f.id && payerDefaults ? (
        ...
        <>
          ...
          <FieldRow label="Bank Account Holder Name (optional)">
  ...
  const secPreview = (
    <div className="space-y-3">
      <div className="rounded-xl border border-border bg-muted/30 p-3 grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-2 text-sm">
        <SummaryRow label="Workshop" value={f.name || "—"} />
        <SummaryRow label="Date & time" value={[f.event_date, f.event_time].filter(Boolean).join(" · ") || "—"} />
        <SummaryRow label="Location" value={[f.venue, f.city].filter(Boolean).join(", ") || "—"} />
        <SummaryRow label="Sessions" value={(f.session_schedule ?? []).length ? (f.session_schedule ?? []).map((s: any) => [s.time, s.name].filter(Boolean).join(" ")).join(" · ") : "—"} />
        <SummaryRow label="Single price" value={f.price_inr ? `₹${f.price_inr}` : "—"} />
  ...
  const sections: FormSection[] = [
    { id: "basic", label: "Basic Information", desc: "Title, description, date and type", done: !!f.name, node: secBasic },
    { id: "registration", label: "Registration", desc: "Method, open date and capacity", done: true, node: secRegistration },
    { id: "sessions", label: "Sessions", desc: "Songs / classes with timings", done: (f.session_schedule ?? []).length > 0, node: secSessions },
    { id: "pricing", label: "Pricing", desc: "Single and on-the-spot prices", done: !!f.price_inr, node: secPricing },
    { id: "silver", label: "Silver Seat", desc: "Premium seat price and capacity", done: !f.silver_seat_enabled || !!f.silver_seat_price, node: secSilver },
